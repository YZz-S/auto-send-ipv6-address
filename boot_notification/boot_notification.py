...
def log_message(message):
    ...
    config = load_config()
    if not config:
        log_file = "boot_notification.log"
    else:
        log_file = config["paths"].get("log_file", "boot_notification.log")
...
def send_boot_notification(config):
    """发送开机通知邮件"""
    try:
        # 获取主机名
        ...
        # 创建邮件主题和正文
        subject = f"[{hostname}] 系统启动通知"
        body = f"""设备 {hostname} 已启动
...
"""
        ...
        log_message(f"开机通知邮件已成功发送至 {config['receiver_email']}")
        return True
    except Exception as e:
        log_message(f"发送开机通知邮件失败: {e}")
        import traceback
